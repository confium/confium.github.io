...
Confium's `confium-coordinator` serves as a gossip endpoint for
institutional PKI deployments. Each institutional coordinator
shares its tree head with the institutional anchor's coordinator,
which acts as the global witness.
...
The OTS client in `confium-transparency` supports this.

## Recommended high-assurance deployment

For institutional PKI (highest assurance):
